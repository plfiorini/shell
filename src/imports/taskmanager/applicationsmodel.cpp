...
ApplicationsModel::ApplicationsModel(QObject *parent)
    ...
    , m_manager(new WlrForeignToplevelManagerV1)
{
    // Pinning settings
    ...
    connect(m_manager, &WlrForeignToplevelManagerV1::toplevel, this, [this](WlrForeignToplevelHandleV1 *handle) {
        if (!handle->appId().isEmpty()) {
            auto *app = addApplication(handle->appId());
            app->addHandle(handle);
        }

        connect(handle, &WlrForeignToplevelHandleV1::appIdChanged,
                this, &ApplicationsModel::handleAppIdChanged);
        connect(handle, &WlrForeignToplevelHandleV1::maximizedChanged, this, [this, handle](bool maximized) {
        ...
        connect(handle, &WlrForeignToplevelHandleV1::fullscreenChanged, this, [this, handle](bool fullscreen) {
        ...
        connect(handle, &WlrForeignToplevelHandleV1::activeChanged, this, [this, handle](bool activated) {
...
void ApplicationsModel::handleAppIdChanged(const QString &oldAppId, const QString &newAppId)
{
    ...
    auto *handle = qobject_cast<WlrForeignToplevelHandleV1 *>(sender());
    if (!handle)
        return;
