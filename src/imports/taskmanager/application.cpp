...
void Application::activate()
{
    ...
    // Only Qt >= 5.14 gives us access to wl_seat
...
#else
    qCWarning(lcTaskManager, "Activate is not available with Qt < 5.14");
#endif
}

bool Application::launch(const QStringList &urls)
{
    qWarning()<<m_desktopFile->fileName()<<isRunning();
...
void Application::addHandle(WlrForeignToplevelHandleV1 *handle)
{
    connect(handle, &WlrForeignToplevelHandleV1::closed,
...
void Application::removeHandle(WlrForeignToplevelHandleV1 *handle)
{
    disconnect(handle, &WlrForeignToplevelHandleV1::closed,
               this, &Application::handleClosed);
